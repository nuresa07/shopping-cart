  ...
  <div class="container">
    <div class="user-profile">
      ...
      <div class="flex">
        ...
        <a onclick="return confirm('are you sure you went to logout');" href="index.php?logout=<?= $user_id; ?>" class="delete-btn">logout</a>
      </div>
    </div>
        ...
        <tbody>
          ...
          if (mysqli_num_rows($cart_query) > 0) {
            while ($fetch_cart = mysqli_fetch_assoc($cart_query)) { ?>
              <tr>
                <td><img src="img/<?= $fetch_cart['image'] ?>" height="100" alt=""></td>
                <td><?= $fetch_cart['name'] ?></td>
                <td>$<?= $fetch_cart['price'] ?>/-</td>
                <td>
                  <form action="" method="post">
                    <input type="hidden" name="cart_id" value="<?= $fetch_cart['id']; ?>">
                    <input type="number" name="cart_quantity" value="<?= $fetch_cart['quantity'] ?>" min="1">
                    <input type="submit" name="update_cart" value="update" class="option-btn">
                  </form>
                </td>
                <td>$<?= $sub_total = number_format($fetch_cart['price'] * $fetch_cart["quantity"]) ?>/-</td>
                <td><a href="index.php?remove=<?= $fetch_cart['id']; ?>" class="delete-btn" onclick="return confirm('remove ?')">delete</a></td>
              </tr>
          ...
          <tr class="table-button">
            ...
            <td><?= $grand_total ?>/-</td>
            <td>
              <a href="index.php?delete_all" onclick="return confirm('delete all from cart?')" class="delete-btn <?= ($grand_total > 1) ? '' : 'disabled' ?>">delete all</a>
            </td>
          </tr>
        </tbody>
      </table>
